	...
	private int HookProc(int hdlg, int msg, int wParam, int lParam)
	{
		switch (msg)
		{
			case WM_INITDIALOG:
                ...
                ////create the label
                //labelHandle = CreateWindowEx(0, "STATIC", "mylabel", WS_VISIBLE | WS_CHILD | WS_TABSTOP, point.X, point.Y + 12, 200, 100, parent, 0, 0, 0);
                //SetWindowText(labelHandle, "&Override?");

                //fontHandle = SendMessage(fileTypeWindow, WM_GETFONT, 0, 0);
                //SendMessage(labelHandle, WM_SETFONT, fontHandle, 0);
                ...
                //point = new POINT();
                //point.X = aboveRect.Left + (int)((aboveRect.Right - aboveRect.Left) * 0.9);
                //point.Y = aboveRect.Bottom;
                //ScreenToClient(parent, ref point);
                ...
                //int checkHandle = CreateWindowEx(0, "CheckBox", "mycheckbox", WS_VISIBLE | WS_CHILD | CBS_HASSTRINGS | CBS_DROPDOWNLIST | WS_TABSTOP, point.X, point.Y + 8, rightPoint.X - point.X, 100, parent, 0, 0, 0);
                //SendMessage(checkHandle, WM_SETFONT, fontHandle, 0);

                ////and add the encodings we want to offer
                //SendMessage(checkHandle, CB_ADDSTRING, 0, "UTF-8");
                //SendMessage(checkHandle, CB_ADDSTRING, 0, "UTF-8 with preamble");
                //SendMessage(checkHandle, CB_ADDSTRING, 0, "Unicode");
                //SendMessage(checkHandle, CB_ADDSTRING, 0, "ANSI");

                //SendMessage(checkHandle, CB_SETCURSEL, (int)m_EncodingType, 0);
...
//                //create the label
//                int labelHandle = CreateWindowEx(0, "STATIC", "Load dialog", WS_VISIBLE | WS_CHILD | WS_TABSTOP, point.X, point.Y + 12, 200, 100, parent, 0, 0, 0);
//                SetWindowText(labelHandle, "&Encoding:");
